        ...
        success: function(response) {
            ...
            instruments.forEach(function (instrument) {
                htmlContent += `
                    ...
                    <a href="#" class="instrument-link" 
                       data-category-id="${instrument.categoryId}" 
                       data-instrument-name="${instrument.instrumentName}" 
            ...
            $('.instrument-link').on('click', function(e) {
                ...
                // Remove active class from previously clicked links
                $('.instrument-link').removeClass('active-instrument');
    
                // Add active class to the clicked link
                $(this).addClass('active-instrument');
    
                var categoryId = $(this).data('category-id');
                var InstrumentName = $(this).data('instrument-name');
                var InstrumentStatus = $(this).data('is-active') ? 'true' : 'false';
    
                if (InstrumentName.length > 20) {                  
                    var splitIndex = InstrumentName.indexOf(' ', 20);
                    if (splitIndex === -1 || splitIndex > 40) {                         
                        splitIndex = 20;
                    }
                    InstrumentName = InstrumentName.substring(0, splitIndex) + " " + InstrumentName.substring(splitIndex + 1);
                }
    
                $('#InstrumentName').text(InstrumentName);
    
                if (InstrumentStatus) {
                    $('#InstrumentStatus').removeClass('alert-danger').addClass('alert-success').html('<strong>This Machine is available for use.</strong>');
                } else {
                    $('#InstrumentStatus').removeClass('alert-success').addClass('alert-danger').html('<strong>This Machine is under maintenance. Sorry for the inconvenience.</strong>');
                }
